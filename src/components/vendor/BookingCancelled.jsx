...
import { bookingDetails, bookingDetailsCancelled} from '../../services/turfAxios';
...
function BookingCancelled() {
    ...
    useEffect(() => {
        const getBookings = async () => {
          ...
          try {
            setLoading(true);
            const  data =await  bookingDetailsCancelled()
  ...
  return (
     <div>
      <div className="w-full bg-slate-950 opacity-90 min-h-screen  px-7 py-10 mt-16 ">
      <div className="p-1 rounded-lg dark:border-gray-700 mt-1">
          {loading ? (
                  ...
                  <tbody>
                    {
                      bookings.map((per,index) => (
                        ...
                        <td class="px-6 py-4">{per.turfId?.venueName} </td>
                        <td class="px-6 py-4">{per.userId?.name} </td>
