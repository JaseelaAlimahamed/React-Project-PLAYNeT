...
import { setSport, setFacility, clearBooking } from "../../redux/slices/turf/bookingSlice";
import swal from "sweetalert";
import { updateWallet } from "../../redux/slices/userSlice";
import toast from "react-hot-toast";
import { EmptyCart } from './images/Icons';

import BookingCalendar from "./BookingCalender";
import { userBook, verifyPayment } from "../../services/userAxios";
import is from "date-fns/esm/locale/is/index.js";


function BookingSection( turf ) {

const turfDetail=turf.turf
const details=turfDetail.sportFacility
  ...
  function showPaymentOptions() {
    ...
    swal({
      ...
      text: `Choose your preferred payment option, wallet balance Rs.${wallet}.00 , amount to be paid in online ${
        turfDetail.actualPrice - turfDetail.actualPrice * (turfDetail.discountPercentage / 100) - wallet < 0 ? "0" : turfDetail.actualPrice - turfDetail.actualPrice * (turfDetail.discountPercentage / 100) - wallet
      }`,
      buttons: {
        offline: {
          text: "include wallet",
  ...
  async function handleBooknow(method) {
    ...
    try {
      let response = await userBook({ turf: turfDetail._id, method, sport: sportAndFacility.sport, facility: sportAndFacility.facility, slotDate: sportAndFacility.date, slotTime: sportAndFacility.slot });
  ...
  function initPayment(datas) {
    ...
    try {
      const options = {
        key: 'rzp_test_3qOFRfWWDf28ml',
        amount: datas.amount,
        currency: datas.currency,
        name: turfDetail.venueName,
        description: "payment for book a slot",
        image: turfDetail.image,
        order_id: datas.id,
        handler: async (response) => {
          ...
          try {
            ...
            const { data } = await verifyPayment(detail)
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2  xl:grid-cols-4 px-4 py-5">
                    {details && details.length > 0 ? (
                      details.map((per, index) => (
                        ...
                        // Render this content when turf.sportFacility doesn't exist or has a length of zero
                  ...
                  turfDetail.slots?.length && (
                    <div className="bg-white rounded-lg">
                      ...
                      <div className=" px-4 py-5 space-x-9 ">
                        <BookingCalendar slots={turfDetail.slots} turfId={turfDetail._id} />
                      </div>
                    </div>
                  )
                )}
                  ...
                  <div className="md:basis-4/12 bg-white h-80 rounded-lg my-4 md:my-11 grid p-2 relative">
                    <div className="xl:text-2xl h-16 rounded bg-[#a7b4ca3c] flex items-start justify-between px-2 w-full lg:text-xl text-sm">
                      ...
                      <div className="flex flex-col items-end">
                        ...
                        <p className="text-sm">₹{turfDetail.actualPrice - (turfDetail.actualPrice * turfDetail.discountPercentage) / 100}</p>
                      </div>
                    </div>
                    <div className="bg-green-800 absolute bottom-0 rounded-b-md text-xl w-full">
